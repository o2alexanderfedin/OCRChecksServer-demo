...
describe('MistralJson Semi-Integration', () => {
    ...
    beforeAll(() => {
        console.log('Initializing MistralJsonExtractorProvider with real dependencies')
        
        // Create real IO instance
        const io: IoE = {
            fetch: global.fetch,
            atob: global.atob,
            console,
        ...
        const apiKey = process.env.MISTRAL_API_KEY || 'vYS1jOH55qvFc5Qqzgn2JHXN3cjMCJQp' // Default for test
    ...
    it('should extract structured JSON from utility bill markdown', async () => {
        ...
        // Check that charges are extracted correctly
        if (result[1].json.charges) {
            expect(result[1].json.charges.electricity).toBeDefined()
            expect(result[1].json.charges.water).toBeDefined()
        }
